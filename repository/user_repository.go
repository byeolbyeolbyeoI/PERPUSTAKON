...
func (s *UserRepository) AddUser(user models.User) *APIError.APIError {
	var dbUser models.User
	// do the query, user only need read permission, there is change in dbUser
	...
	if err != nil { // if there is an error
		...
		_, err = s.DB.Exec("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", user.Username, hashedPassword, user.Role)
		if err != nil {
			return APIError.NewAPIError(fiber.StatusInternalServerError, "Error adding user", err.Error())
		}
